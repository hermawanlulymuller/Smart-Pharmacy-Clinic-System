 ...
 * Version: 3.5.0 Enterprise REST API Database Engine
 * ============================================================================
 * 
...
/**
 * MENGINISIALISASI SHEET & MEMASUKKAN DATA DUMMY SECARA OTOMATIS
 * Jalankan fungsi ini pertama kali dari Apps Script Editor!
 */
function setupDatabase() {
  ...
  sheetsNeeded.forEach(sheetName => {
    ...
    // Clear existing data & formatting
    sheet.clear();
  ...
  Logger.log("SUCCESS: 11 Sheet telah dibuat & Data Dummy berhasil dimasukkan!");
  return "Database Initialized Successfully!";
}

/**
 * Google Sheet Custom Menu Handler
 */
function onOpen() {
  const ui = SpreadsheetApp.getUi();
  ui.createMenu('Smart Clinic Setup')
...
function doGet(e) {
  ...
  if (action === "ping") {
    return createJsonResponse({ status: "success", message: "Google Sheet Database API Ready & Active!", timestamp: new Date() });
  }
...
function doPost(e) {
  try {
    ...
    if (action === "insert") {
      ...
      return createJsonResponse({ status: "success", message: "Record inserted into Google Sheet!", data: rowData });
    }

    if (action === "syncAll" || action === "setupWithDummy") {
      setupDatabase();
      return createJsonResponse({ status: "success", message: "Full database populated with initial dummy records!" });
    }
